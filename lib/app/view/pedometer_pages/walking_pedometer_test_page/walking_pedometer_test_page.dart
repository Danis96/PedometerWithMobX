...
class WalkingPedometerTestPage extends StatefulWidget {
  const WalkingPedometerTestPage(this.store, {Key? key}) : super(key: key);

  final PedometerStore store;

  @override
  State<WalkingPedometerTestPage> createState() => _WalkingPedometerTestPageState();
}

class _WalkingPedometerTestPageState extends State<WalkingPedometerTestPage> {
  ...
  Future<void> startTimer() async {
    await widget.store.startTracking().then((value) {
  ...
  Widget _buildBody(BuildContext context) {
    return SizedBox(
      ...
      child: ListView(shrinkWrap: true, children: [_buildHeadline(context)]),
    );
  }
  ...
  Widget _buildTimerView(BuildContext context) {
    ...
    if (minutes == '00' && seconds == '00') {
      ...
      testFinished = true;
      widget.store.stopTracking();
      widget.store.setUsersDistance(email!, widget.store.distanceTraveled.toInt());
    }
    return Center(
